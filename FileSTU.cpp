...
{ public:
        ...
        {cout<<"Enter Name of the Student:"; cin>>name;
         ...
         cout<<"Enter Student's Class:"; cin>>Class;
...
{ char ans='Y';
  ...
  while(ans=='Y'||ans=='y')
   {  S.getdata();
      ...
      cout<<"\n Want to enter more records?(y/n):"; cin>>ans;
   }
   filin.seekp(0);
   filin.close();
 }
 ...
 {  int rn; 
    ifstream fout("Stu.txt",ios::in); fout.seekg(0);
    cout<<"Enter rollno to be searched for:";cin>>rn;
    while(!fout.eof())
    { fout.read((char*)&S, sizeof(student));
      if (S.rollno==rn) {S.putdata(); break;}
...
{  // Interface
   ...
   cout<<"\n Enter your choice for Act:"; cin>>choice;
   switch(choice)
   {   case 1:input();
              break; //function call to input datas of student using class
       case 2:search();//function call to search student details using roll no of class 
              break;
       case 3:display();       
   }
 return 0;
